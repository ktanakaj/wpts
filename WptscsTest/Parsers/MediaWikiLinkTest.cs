...
namespace Honememo.Wptscs.Parsers
{
    ...
    public class MediaWikiLinkTest
    {
        ...
        public void TestTryParseBasic()
        {
            ...
            MediaWikiParser parser = new MediaWikiParser(new MediaWiki(new Language("en")));

            // タイトルのみ
            Assert.IsTrue(MediaWikiLink.TryParse("[[testtitle]]", parser, out element));
            ...
            // タイトルとセクション
            Assert.IsTrue(MediaWikiLink.TryParse("[[testtitle#testsection]]", parser, out element));
            Assert.AreEqual("testtitle", element.Title);
            ...
            // タイトルとセクションとパイプ後の文字列
            Assert.IsTrue(MediaWikiLink.TryParse("[[testtitle#testsection|testpipe1|testpipe2]]", parser, out element));
            Assert.AreEqual("testtitle", element.Title);
            Assert.AreEqual("testsection", element.Section);
            ...
            // タイトルとセクションとパイプ後の文字列とコード
            Assert.IsTrue(MediaWikiLink.TryParse("[[en:testtitle#testsection|testpipe1|testpipe2]]", parser, out element));
            Assert.AreEqual("testtitle", element.Title);
            Assert.AreEqual("testsection", element.Section);
            Assert.AreEqual(2, element.PipeTexts.Count);
            Assert.AreEqual("testpipe1", element.PipeTexts[0].ToString());
        ...
        public void TestTryParseNg()
        {
            ...
            // 開始タグが無い
            Assert.IsFalse(MediaWikiLink.TryParse("testtitle]]", parser, out element));
        ...
        {
            MediaWikiLink element;
            MediaWikiParser parser = new MediaWikiParser(new MediaWiki(new Language("ja")));

            // カテゴリ標準
            Assert.IsTrue(MediaWikiLink.TryParse("[[Category:test]]", parser, out element));
            ...
            // カテゴリソート名指定
            Assert.IsTrue(MediaWikiLink.TryParse("[[Category:test|てすと]]", parser, out element));
            Assert.AreEqual("Category:test", element.Title);
            Assert.AreEqual(1, element.PipeTexts.Count);
            ...
            Assert.IsTrue(MediaWikiLink.TryParse("[[ファイル:test.png|thumb|100px|テスト[[画像]]]]", parser, out element));
