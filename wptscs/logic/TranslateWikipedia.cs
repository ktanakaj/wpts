...
namespace wptscs.logic
{
    ...
    {
		// コンストラクタ
		public TranslateWikipedia(WikipediaInformation i_Source, WikipediaInformation i_Target)
        ...
        protected override bool runBody(String i_Name)
        {
	        ...
	        // 新しい言語間リンクと、コメントを追記
	        Text += ("\n\n[[" + source.Code + ":" + i_Name + "]]\n");
	        Text += (String.Format(Resources.ArticleFooter, MYAPP.Cmn.GetProductName(),
        ...
        protected String getInterWiki(String i_Name)
        {
        ...
        // 渡されたテキストを解析し、言語間リンク・見出し等の変換を行う
        protected String replaceText(String i_Text, String i_Parent)
        {
	        ...
	        if(!String.IsNullOrEmpty(link.Article) &&
	           !(link.Article == i_Parent && String.IsNullOrEmpty(link.Code) && !String.IsNullOrEmpty(link.Section))){
		        ...
		        // 表示名が存在しない場合、元の名前を表示名に設定
			        ...
			        MYAPP.Cmn.AddArray(ref link.PipeTexts, article.Title);
		        }
	        }
	        // 見出し（[[#関連項目]]とか）を出力
