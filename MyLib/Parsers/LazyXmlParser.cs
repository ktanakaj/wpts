...
namespace Honememo.Parsers
{
    ...
    /// </remarks>
    public class LazyXmlParser : AbstractParser
    {
        #region 定数宣言
        ...
        /// </remarks>
        public bool TryParseTag(string s, out XmlElement result)
        {
            // 入力値確認。タグでない場合は即終了
            result = null;
            ...
            if (s[index - 1] == '/')
            {
                IList<IElement> innerElements = new List<IElement>();
                innerElements.Add(new TextElement(s.Substring(0, index + 1)));
                result = new XmlElement(name, attribute, innerElements);
                return true;
            }
            ...
            IList<IElement> innerElements2 = new List<IElement>();
            innerElements2.Add(new TextElement(s.Substring(0, index + 1)));
            result = new XmlElement(name, attribute, innerElements2);
            return true;
        }
        ...
        protected override bool TryParseElements(string s, out IElement result)
        {
            ...
            else if (XmlElement.TryParseLazy(s, this, out xmlElement))
            {
                result = xmlElement;
                return true;
            }
